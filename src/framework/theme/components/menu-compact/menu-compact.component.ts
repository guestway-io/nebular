...
export enum NbToggleStatesMenuCompact {
  Expanded = 'expanded',
  Collapsed = 'collapsed',
}
...
export class NbMenuItemCompactComponent implements DoCheck, AfterViewInit, OnDestroy {
  ...
  @Input() badge: NbMenuBadgeConfigCompact;
  ...
  toggleState: NbToggleStatesMenuCompact;
  ...
  ngDoCheck() {
    this.toggleState = this.menuItem.expanded
      ? NbToggleStatesMenuCompact.Expanded
      : NbToggleStatesMenuCompact.Collapsed;
  }
  ...
  ngAfterViewInit() {
    this.menuService
      ...
      .pipe(
        ...
        map(({ item }: NbMenuBagCompact) => item.expanded),
        takeUntil(this.destroy$),
      )
      .subscribe(
        (isExpanded) =>
          (this.toggleState = isExpanded ? NbToggleStatesMenuCompact.Expanded : NbToggleStatesMenuCompact.Collapsed),
      );
  }
...
export class NbMenuCompactComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  /**
   * List of menu items.
   * @type List<NbMenuItemCompact> | List<any> | any
   */
  ...
  ngOnInit() {
    ...
    this.menuInternalService
      .onGetSelectedItem()
      .pipe(
        filter((data: { tag: string; listener: BehaviorSubject<NbMenuBagCompact> }) => this.compareTag(data.tag)),
        takeUntil(this.destroy$),
      )
      .subscribe((data: { tag: string; listener: BehaviorSubject<NbMenuBagCompact> }) => {
