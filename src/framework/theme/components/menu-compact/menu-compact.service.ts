...
export interface NbMenuBagCompact {
  tag: string;
  item: NbMenuItemCompact;
}

const itemClick$ = new Subject<NbMenuBagCompact>();
const addItems$ = new ReplaySubject<{ tag: string; items: NbMenuItemCompact[] }>(1);
const navigateHome$ = new ReplaySubject<{ tag: string }>(1);
const getSelectedItem$ = new ReplaySubject<{ tag: string; listener: BehaviorSubject<NbMenuBagCompact> }>(1);
const itemSelect$ = new ReplaySubject<NbMenuBagCompact>(1);
const itemHover$ = new ReplaySubject<NbMenuBagCompact>(1);
const submenuToggle$ = new ReplaySubject<NbMenuBagCompact>(1);
const collapseAll$ = new ReplaySubject<{ tag: string }>(1);

export type NbMenuBadgeConfigCompact = Omit<NbBadge, 'position'>;
...
export class NbMenuItemCompact {
   ...
   * @type {boolean}
   */
  badge?: NbMenuBadgeConfigCompact;
  /**
   * Children items
   * @type {List<NbMenuItemCompact>}
   */
...
export class NbMenuCompactService {
   ...
   */
  getSelectedItem(tag?: string): Observable<NbMenuBagCompact> {
    const listener = new BehaviorSubject<NbMenuBagCompact>(null);
  ...
  onItemClick(): Observable<NbMenuBagCompact> {
    return itemClick$.pipe(share());
  }

  onItemSelect(): Observable<NbMenuBagCompact> {
    return itemSelect$.pipe(share());
  }

  onItemHover(): Observable<NbMenuBagCompact> {
    return itemHover$.pipe(share());
  }

  onSubmenuToggle(): Observable<NbMenuBagCompact> {
...
export class NbMenuCompactInternalService {
  ...
  onGetSelectedItem(): Observable<{ tag: string; listener: BehaviorSubject<NbMenuBagCompact> }> {
